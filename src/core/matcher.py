...
class PrecedentMatcher:
    def __init__(
        self, qdrant_client, collection_name: str, model_name: str = "all-MiniLM-L6-v2"
    ):
    ...
    def match_precedent(
        ...
        for result in all_results:
            result_id = result["id"]
            if (
                result_id not in unique_results
                or result["score"] > unique_results[result_id]["score"]
            ):
        ...
        all_results.sort(
            key=lambda x: (x["score"], x.get("rerank_score", 0)), reverse=True
        )
